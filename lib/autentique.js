...
class AutentiqueClient {
  ...
  async createDocument(documentName, pdfBuffer, signers) {
    try {
      ...
      // Log signature links for each signer
      document.signatures.forEach(signature => {
        console.log(`Signature link for ${signature.email}: ${signature.link.short_link}`);
      });
  ...
  async sendContractForSignature(googleDocId, documentName, leadContactEmail, leadContactName = null) {
    try {
      ...
      return {
        ...
        // Return the first signature link as the primary link to share
        primaryLink: document.signatures[0]?.link?.short_link || null,
      };
    } catch (error) {
